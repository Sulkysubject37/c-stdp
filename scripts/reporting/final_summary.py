...
def generate_final_summary():
    ...
    if os.path.exists("results/parameter_sensitivity.csv"):
        df = pd.read_csv("results/parameter_sensitivity.csv")
        best_f1 = df["F1"].max()
    ...
    if os.path.exists("results/delay_stress_test.csv"):
        df = pd.read_csv("results/delay_stress_test.csv")
    ...
    summary_df.to_csv("results/final_metrics_summary.csv", index=False)
    
    # Generate Markdown table for report
    md_table = summary_df.to_markdown(index=False)
    with open("results/final_metrics_summary.md", "w") as f:
        f.write(md_table)
